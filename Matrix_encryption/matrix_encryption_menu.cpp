...
string e(string text) {  
    string result = ""; 
    int a; a = sqrt(text.length());  
    char A[1000][1000]; 
    int i, j, n = 0; 
  
    for (int arr = 0; arr < sqrt(text.length()); arr++) 
    { 
        for (int idx = 0; idx < sqrt(text.length()); idx++) 
        { 
            A[arr][idx] = text[idx + (arr * sqrt(text.length()))]; 
        } 
    } 
if (a % 2 == 1) { 
    i = sqrt(text.length()) / 2, j = sqrt(text.length()) / 2; 
    do { 
        n++; 
        for (int k = 0; k < n; k++) result.push_back(A[i][j--]); 
        if (n == sqrt(text.length())) break; 
        for (int k = 0; k < n; k++) result.push_back(A[i++][j]); 
        n++; 
        for (int k = 0; k < n; k++) result.push_back(A[i][j++]); 
        for (int k = 0; k < n; k++) result.push_back(A[i--][j]); 
    ...
    i = sqrt(text.length()) / 2, j = (sqrt(text.length()) / 2) - 1; 
    do { 
        n++; 
        for (int k = 0; k < n; k++) result.push_back(A[i][j++]); 
        for (int k = 0; k < n; k++) result.push_back(A[i--][j]); 
